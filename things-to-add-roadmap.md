...
|---|---|---|---|---|
| P0 | Rack/Attack misses and retries | **Early-Enter experiment disproven; rebuild pending.** Native evidence showed Enter is discarded while a long-word interrupt owns input. Lua now arms each complete valid selection and emits one `ATTACK_READY` on the first subsequent native update after `DIALOG_INACTIVE`; the former extra 15-update mouse delay remains removed. | Preserve tile-selection focus and submit with Enter from one deterministic post-presentation event—no Attack pulses and no arbitrary sleep. | Rebuild and run a fresh Chapter 1 with no acknowledgement retries; treat the mandatory long-word presentation as engine time. |
