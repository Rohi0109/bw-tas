...
|---|---|---|---|---|
| P0 | Rack/Attack misses and retries | **Exact native predicate implemented; rebuild pending.** Historical word-presentation events no longer roll back an authorized submission. A live `BROWNING` trace then proved `DIALOG_INACTIVE` can precede actual input ownership; readiness now also requires the battle state `BE_IDLE` and player state `CREATURE_IDLE`, matching the checks guarding Attack in `BattleEngine:MouseDown`. | Preserve tile-selection focus and submit once with Enter from the complete native MouseDown ownership predicate. | Run a fresh Chapter 1 with one decision and one native submission per enemy, with no acknowledgement retries. |
