...
|---|---|---|---|---|
| P0 | Rack/Attack misses and retries | **Rebuilt; live acceptance pending.** Enter submission is reliable. `BattleEngine:CanSubmitTiles()` was tested and proven to be an unimplemented one-instruction stub, which suppressed every readiness event. The replacement emits on the first native update after the presentation and grid overlay release input, without the former 15-update mouse delay. | Preserve tile-selection focus, submit with Enter, and authorize from the concrete dialogue/grid ownership fields. | Run a fresh Chapter 1 with immediate post-presentation submission and no acknowledgement retries. |
