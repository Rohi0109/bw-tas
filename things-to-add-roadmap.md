...
|---|---|---|---|---|
| P0 | Rack/Attack misses and retries | **Next-update handshake implemented; rebuild pending.** Enter submission is reliable. Lua now arms each new complete valid selection and emits one `ATTACK_READY` on the following native update. A 7+ letter word-presentation interrupt is explicitly allowed; unrelated dialogue, grid overlays, and dead-enemy transitions still block submission. | Preserve tile-selection focus and submit with Enter from one deterministic next-update event—no Attack pulses and no arbitrary sleep. | Rebuild and run a fresh Chapter 1 with no acknowledgement retries and no wait for long-word presentation exit. |
