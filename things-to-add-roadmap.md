...
| P0 | Move chapter-boss menu exit to the earliest save-safe lethal edge | Angry Mob reached native zero HP at **12:33:37.134**, but `BOSS_RESET_READY` and the menu reset did not occur until **12:33:41.421**, after the complete 4.287-second death animation. The current reset is reliable but is not an animation skip. | Instrument accepted lethal submission, HP-zero, save/progress mutation, battle-menu ownership, death frames, and chapter-map state. Identify the earliest frame where progress survives a menu exit, then authorize the reset from that exact native condition instead of the settled-animation edge. | Repeated same-seed boss kills exit before the visible death animation completes, retain the unlocked next chapter and treasures, and never replay the defeated boss. |
