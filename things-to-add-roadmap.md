...
|---|---|---|---|---|
| P0 | Rack/Attack misses and retries | **Enter fix live-confirmed; fresh-run acceptance pending.** The stranded `TURNPIKES` submitted immediately with Enter and emitted `AUTOMATION_ATTACK_SUBMITTED=Specter`. A short word then exposed an asynchronous refocus race, so Attack submission no longer changes focus at the native-ready edge. | Keep the native readiness edge, preserve the focus established while selecting tiles, and submit with Enter rather than a coordinate click. | Run a fresh Chapter 1 without acknowledgement retries. |
