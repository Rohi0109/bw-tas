"""Native X11 input and calibrated UI controls; no word-solving policy."""
...
class XEvent(ctypes.Union):
    _fields_ = [("client", XClientMessageEvent), ("pad", ctypes.c_long * 24)]
...
class X11Keyboard:
    ...
    def focus(self) -> None:
        ...
        self._refresh_window()
        # Raising Mutter's decorative frame does not activate a Wine window
        # over a native Wayland app. Ask the WM to activate the managed client.
        client = self.window
        while client != self.frame:
            parent = self._parent(client)
            if not parent or parent == self.frame:
                break
            client = parent
        ...
        event.client.window = client
        event.client.message_type = self.x11.XInternAtom(self.display, b"_NET_ACTIVE_WINDOW", False)
        event.client.format = 32
        event.client.data.l[0] = 2  # pager/application activation request
        self.x11.XSendEvent(self.display, self.x11.XDefaultRootWindow(self.display),
                           False, (1 << 20) | (1 << 19), ctypes.byref(event))
        self.x11.XMapRaised(self.display, self.frame)
        self.x11.XSync(self.display, False)
        self.x11.XSetInputFocus(self.display, client, 1, 0)
    ...
    def click(self, x: int, y: int, delay: float) -> None:
        ...
        self.x11.XFlush(self.display)
        # Keep the host-side delivery edge separate from the configured guard.
        # Native screen ownership is still established only by telemetry or a
        # future validated observer; this timestamp is input evidence only.
        self.last_input_flush_at = time.monotonic()
        time.sleep(delay)
    ...
    def select_word(
        ...
        for offset, letter in enumerate(word.upper()):
            ...
            if confirm_tile is not None:
                attempts = 1
                confirmed = confirm_tile(offset + 1)
